...
import com.google.android.stardroid.renderer.ImageObjectManager;
import com.google.android.stardroid.renderer.RendererController;
import com.google.android.stardroid.renderer.RendererControllerBase.ImageManager;
...
public class DynamicStarMapActivity extends Activity implements OnSharedPreferenceChangeListener
{
	...
	private EditText x_jump;
	private EditText y_jump;
	private EditText z_jump;
	private EditText x_search;
	private EditText y_search;
	private EditText z_search;
	private float ZOOMRATIO = 0.8f;
	...
	public void onCreate(Bundle icicle)
	{
		...
		layerManager = StardroidApplication.getLayerManager(getAssets(), sharedPreferences, getResources(), this);
		...
		ActivityLightLevelChanger activityLightLevelChanger = new ActivityLightLevelChanger(this, new NightModeable() {
			@Override
	...
	public GeocentricCoordinates convertAxis(float x, float y, float z)
	{
		float X = (float) (Math.cos(x) * Math.cos(y));
		float Y = (float) (Math.cos(x) * Math.sin(y));
	...
	public boolean onOptionsItemSelected(MenuItem item)
	{
		...
		case R.id.menu_item_dim:
			...
			nightMode = !nightMode;
			sharedPreferences.edit().putString(ActivityLightLevelManager.LIGHT_MODE_KEY, nightMode ? "NIGHT" : "DAY").commit();
			Analytics.getInstance(this).trackEvent(Analytics.USER_ACTION_CATEGORY, Analytics.MENU_ITEM, Analytics.TOGGLED_NIGHT_MODE_LABEL, nightMode ? 1 : 0);
			break;
	...
	private enum SessionBucketLength {
		LESS_THAN_TEN_SECS(10), TEN_SECS_TO_THIRTY_SECS(30), THIRTY_SECS_TO_ONE_MIN(60), ONE_MIN_TO_FIVE_MINS(300), MORE_THAN_FIVE_MINS(Integer.MAX_VALUE);
		private int seconds;
	...
	public void onStop()
	{
		...
		Analytics.getInstance(this).trackEvent(Analytics.GENERAL_CATEGORY, Analytics.SESSION_LENGTH_BUCKET, bucket.toString(), sessionLengthSeconds);
	}
	...
	public void setTimeTravelMode(Date newTime)
	{
		...
		Toast.makeText(this, String.format(getString(R.string.time_travel_start_message_alt), dateFormatter.format(newTime)), Toast.LENGTH_LONG).show();
	...
	private void doSearchWithIntent(Intent searchIntent)
	{
		...
		Analytics.getInstance(this).trackEvent(Analytics.USER_ACTION_CATEGORY, Analytics.SEARCH, "search:" + queryString, results.size() > 0 ? 1 : 0);
		if (results.size() == 0)
		{
	...
	private void initializeModelViewController()
	{
		...
		x_jump = (EditText) findViewById(R.id.x_jump);
		y_jump = (EditText) findViewById(R.id.y_jump);
		z_jump = (EditText) findViewById(R.id.z_jump);

		x_search = (EditText) findViewById(R.id.x_search);
		y_search = (EditText) findViewById(R.id.y_search);
		z_search = (EditText) findViewById(R.id.z_search);
	...
	private void onJumpButton()
	{
		try
		{
			float x = Float.valueOf(x_jump.getText().toString());
			float y = Float.valueOf(y_jump.getText().toString());
			float z = Float.valueOf(z_jump.getText().toString());
			searchTarget = convertAxis(x, y, z);
			// rendererController.queueEnableSearchOverlay(searchTarget.copy(),
			// "test");
			// boolean mode = controller.isAutoMode();
			controller.setAutoMode(false);
			controller.teleport(searchTarget);
			model.setFieldOfView(10);
			controller.zoomBy(ZOOMRATIO);
			// controller.setAutoMode(mode);
			InputMethodManager imm = (InputMethodManager) getApplication().getSystemService(Context.INPUT_METHOD_SERVICE);
			imm.hideSoftInputFromWindow(getCurrentFocus().getWindowToken(), 0);
		} catch (Exception e)
		{
			Toast.makeText(getApplicationContext(), "Fill all box with float value", Toast.LENGTH_SHORT).show();
		}
		// rendererController.queueViewerUpDirection(searchTarget);
	}
	...
	private void wireUpScreenControls()
	{
		...
		gestureDetector = new GestureDetector(new GestureInterpreter(new WidgetFader[] { manualControlFader, layerControlFader, zoomControlFader }, mapMover));
	...
	void activateSearchTarget(GeocentricCoordinates target, final String searchTerm)
	{
		...
		searchPromptText.setText(String.format("%s %s", getString(R.string.search_target_looking_message), searchTerm));
		View searchControlBar = findViewById(R.id.search_control_bar);
		searchControlBar.setVisibility(View.VISIBLE);
	}

	private void onKidButton()
	{
		try
		{
			float x = Float.valueOf(x_search.getText().toString());
			float y = Float.valueOf(y_search.getText().toString());
			float z = Float.valueOf(z_search.getText().toString());
			GeocentricCoordinates geocentricCoordinates = convertAxis(x, y, z);
			controller.setAutoMode(false);
			rendererController.queueEnableImageOverlay(geocentricCoordinates, "");
			controller.teleport(geocentricCoordinates);
		} catch (Exception e)
		{
			Toast.makeText(getApplicationContext(), "Fill all box with float value", Toast.LENGTH_SHORT).show();
		}
